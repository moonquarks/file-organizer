...
btnTranscribe.addEventListener('click', async () => {
  ...
  try {
    ...
    // 2. 解码原始音频数据
    progressText.textContent = '正在解码音频（解析为通道 PCM 数据，可能需要几秒钟）...';
    tempCtx = new (window.AudioContext || window.webkitAudioContext)();
    const nativeBuffer = await tempCtx.decodeAudioData(arrayBuffer);
    
    const durationSec = nativeBuffer.duration;
    
    // 3. 在客户端离线进行重采样（转为 16000Hz 单声道，大幅缩减文件体积，保障安全通过 OpenResty 413 大小限制）
    progressText.textContent = '正在离线转换为 16kHz 单声道（体积压缩约 5~8 倍）...';
    const targetSampleRate = 16000;
    
    const offlineCtx = new OfflineAudioContext(1, Math.floor(durationSec * targetSampleRate), targetSampleRate);
    const bufferSource = offlineCtx.createBufferSource();
    bufferSource.buffer = nativeBuffer;
    bufferSource.connect(offlineCtx.destination);
    bufferSource.start();
    
    const audioBuffer = await offlineCtx.startRendering();
    
    // 4. 计算切片参数（以 10 分钟 = 600 秒为基准）
    ...
    for (let i = 0; i < totalSlices; i++) {
      ...
      const numSamples = Math.floor(segmentDuration * targetSampleRate);
      ...
      // 提取该切片的单声道 PCM Float 数据
      const startSample = Math.floor(startSec * targetSampleRate);
      const monoChannel = audioBuffer.getChannelData(0).subarray(startSample, startSample + numSamples);
      
      // 打包并转化为 WAV 单声道二进制格式 (大幅压缩网络传输数据)
      const wavBlob = exportMonoWAV(monoChannel, targetSampleRate);
...
// WAV 单声道编码辅助工具函数 (适用于压缩传输)
function exportMonoWAV(channelData, sampleRate) {
